...
wget -qS --max-redirect 0 http://bing.com/ 2>&1|grep -oiP 'Date: \K.+' > ~/.tmpdate
#echo "$in" > ~/.tmpdate
in=$(cat ~/.tmpdate)
if [ "$in" == '' ]; then
 echo "found"
...
monthnum=$(echo "$in" | cut -f 3 -d " ";)
daynum=$(echo "$in" | cut -f 2 -d " ";)
year=$(echo "$in" | cut -f 4 -d " ";)
time=$(echo "$in" | cut -f 5 -d " ";)
